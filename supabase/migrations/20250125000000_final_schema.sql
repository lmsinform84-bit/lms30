/*
  # LMS Complete Database Schema - Final
  Single migration file with all tables, RLS policies, and storage buckets
  This file includes all changes and can be run directly without running individual migrations
*/
...
-- Ensure source column exists (for existing tables)
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'newsroom' AND column_name = 'source'
  ) THEN
    ALTER TABLE newsroom ADD COLUMN source TEXT DEFAULT 'internal' CHECK (source IN ('internal', 'external'));
    CREATE INDEX IF NOT EXISTS idx_newsroom_source ON newsroom(source);
  END IF;
END $$;
...
DROP POLICY IF EXISTS "Teachers can view all materials" ON storage.objects;
...
DROP POLICY IF EXISTS "Students can view materials" ON storage.objects;
...
DROP POLICY IF EXISTS "Teachers can update submissions" ON storage.objects;
...
-- NEWSROOM IMAGES
DROP POLICY IF EXISTS "Teachers can upload newsroom images" ON storage.objects;
...
DROP POLICY IF EXISTS "Teachers can update newsroom images" ON storage.objects;
CREATE POLICY "Teachers can update newsroom images" ON storage.objects
  FOR UPDATE USING (
    bucket_id = 'newsroom-images' AND auth.uid() IS NOT NULL
  );

DROP POLICY IF EXISTS "Teachers can delete newsroom images" ON storage.objects;
